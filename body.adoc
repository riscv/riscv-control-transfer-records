...
2+|DEPTH |_WARL_ |LCOFIFRZ |BPFRZ |RASEMU |STE |MTE
|===
...
The sctrcontrol register provides access to a subset of mctrcontrol. It is accessible from S-mode and VS-mode, though VS-mode accesses are redirected to vsctrcontrol.
...
|===
2+|19:16 |15:13 |12 |11 |10 |9 |8
2+|DEPTH |_WARL_ |LCOFIFRZ |BPFRZ |RASEMU |VSTE |0
|===

[width="100%",options="header",]
|===
|7 4+|6:3 |2 |1 |0
|CLR 4+|_WARL_ |VU |VS |0
|===
...
The mctrstatus register provides access to CTR status information, and is updated by the hardware when CTR is active (in an enabled privilege mode and not frozen).  Reset value is 0.

[width="100%",options="header",cols="30%,10%,10%,20%,30%"]
|===
|31:16 |15 |14 | 13:8 | 7:0
|_WPRI_ |FROZEN |WRAP |_WPRI_ |TOS
|===

[width="100%",cols="15%,85%",options="header",]
|===
|Field |Description
|TOS |Top of Stack.  Indicates the physical CTR array entry to be written next.  Set to 0 on CSR writes that set ctrcontrol.CLR.  Incremented on new transfers recorded, and decremented on qualified returns when ctrcontrol.RASEMU=1.  Wraps on increment when the value is all 1s, and on decrement when the value is 0.

The number of TOS bits supported is implementation-dependent, but must sufficient to index the array at the maximum depth supported (e.g., 4 bits for a max depth of 16).
|WRAP   |TOS has wrapped.  Set when TOS has value depth-1 (where depth is based on ctrcontrol.DEPTH), and a CTR update causes TOS to be incremented.  Cleared when ctrcontrol.CLR is set, and when TOS has value zero and a CTR update causes TOS to be decremented (which only happens on returns when RASEMU=1).
...
[NOTE]
====
_Logical entry 0, accessed via mireg* when miselect=0x200, is always the physical entry preceding the TOS entry (TOS-1 % depth)._
====
[NOTE]
====
_Because the mctrstatus register is updated by hardware, writes should be performed with caution.  If a read-modify-write to mctrstatus is performed while CTR is active, such that a qualified transfer completes between the read and the write, a hardware update could be lost.  It is safest to write mctrcontrol only when CTR is not active._
====
[NOTE]
====
_Exposing the TOS provides a more efficient means for synthesizng CTR entries.  If a qualified control transfer is emulated, the emulator can simply increment the TOS, then write the synthesized record to entry 0.  If a qualified function return is emulated while RASEMU=1, the emulator can clear ctrsource.V for entry 0, then decrement the TOS._

_Exposing the TOS may also allow support for Linux perf's https://lwn.net/Articles/802821[[.underline]#stack stitching#] capability._
====
...
When Smstateen is implemented, the mstateen0.CTR bit controls access to CTR register state from privilege modes less privileged than M-mode.  When mstateen0.CTR=0, attempts from privilege modes less privileged than M-mode to access sctrcontrol, vsctrcontrol, sctrstatus, or sireg* when siselect is in 0x200..0x2FF, raise an illegal instruction exception.  When mstateen0.CTR=1, accesses to CTR register state behave as described in <<_csrs, CSRs>> and <<_entry_registers, Entry Registers>> above.
