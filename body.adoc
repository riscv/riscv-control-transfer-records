...
[%unbreakable]
====
_Because the `sctrstatus` register is updated by hardware, writes should be performed with caution.  If a multi-instruction read-modify-write to `sctrstatus` is performed while CTR is active, such that a qualified transfer, or trap that causes CTR freeze, completes between the read and the write, a hardware update could be lost.  Software may wish to ensure that either `sctrstatus.FROZEN`=1, or that the enable bit in `*ctrcontrol` corresponding to the current privilege mode is 0, before performing a read-modify-write._
...
entries 0..255. When `miselect` holds a value in this range, `mireg` provides access to <<_control_transfer_record_source_ctrsource, `ctrsource`>>, `mireg2` provides access to <<_control_transfer_record_target_ctrtarget, `ctrtarget`>>, and `mireg3` provides access <<_control_transfer_record_source_ctrdata, `ctrdata`>>.  In RV32, `mireg4` accesses `ctrdata[63:32]`, otherwise `mireg4`, `mireg5`, and `mireg6` are read-only 0.
...
`ctrsource` is an MXLEN-bit WARL register that must be able to hold all valid virtual or physical addresses. It need not be able to hold any invalid addresses.  When XLEN < MXLEN, both explicit writes (by software) and implict writes (for recorded transfers) will be zero-extended.
