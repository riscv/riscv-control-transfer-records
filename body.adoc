...
[%unbreakable]
====
_This specification allows an implementation to not preserve CTR entry state across some or all low power states. The supervisor execution environment is recommended to provide this information to the supervisor using discovery mechanisms such as ACPI and device tree to enable the supervisor to preserve such state, if required, before requesting transition to such low power states._
====
