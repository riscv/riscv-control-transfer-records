...
External trap recording depends not only on the target mode, but on any intervening modes.  For instance, recording an external trap from U-mode to M-mode requires that MTE=STE=1.  Similarly, recording an external trap from VU-mode to HS-mode requires that sctrcontrol.STE=vsctrcontrol.STE=1.
