...
Transitions between VS/VU-mode and M/HS-mode are unique in that they effect a change in the active CTR control register, and hence the CTR configuration.  Privilege mode enable bits (M, S, U) and external trap enable bits (MTE and STE, see <<External Traps>>) from both [ms]ctrcontrol and vsctrcontrol are applied.  For all other fields, the value from vsctrcontrol dictates what is recorded for all traps from VS/VU-mode, and all trap returns to VS/VU-mode.
