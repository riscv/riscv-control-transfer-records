...
[NOTE]
====
_Because the ROI of CTR is perceived to be low for RV32 implementations, CTR does not fully support RV32.  While control flow transfers in RV32 can be recorded, RV32 cannot access bits 63:32 in_ `x__ctrcontrol__` _or `ctrdata`.  A future extension could add support for RV32 by adding 3 new CSRs (`mctrcontrolh`, `sctrcontrolh`, and `vsctrcontrolh`), and making `ctrdata`[63:32] accessible via mireg4._
====
