...
Undefined (WPRI) bits must be read-only 0, unless a <<_custom_extensions, custom extension>> is implemented and enabled.
...
The indirect alias CSR(s) used to access to ctrdata depend on the XLEN, as illustrated in the table below.
