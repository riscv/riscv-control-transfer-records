...
[%unbreakable]
--
All fields are optional except for M, S, U, and BPFRZ.  All unimplemented fields are read-only 0, while all implemented fields are writable.  If the Smcofpmf/Sscofpmf extension is implemented, LCOFIFRZ must be writable.
--
...
.4+| Indirect call | JALR _x1_, _rs_ where _rs_ != _x5_
...
| JAL _x5_
| C.JAL
.3+| Indirect jump (without linkage) | JALR _x0_, _rs_ where _rs_ != (_x1_ or _x5_)
