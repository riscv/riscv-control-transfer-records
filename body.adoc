...
[%unbreakable]
====
_When CTR depth is increased, it is recommended that software then execute CTRCLEAR, or write to all accessible entries, in order to ensure that all entries contain deterministic values._  

_If CTR depth is decreased on a context switch, such that the new context could later increase the depth, the context switch flow should perform CTRCLEAR to ensure that records from the original context cannot be leaked via the entries above the new depth-1.  CTR depth can be changed by M-mode, and by S-mode if `mstateen0`.CTR=1 (see <<_state_enable_access_control, State Enable Access Control>>)._
====
...
_The TG considered making `vsctrcontrol` pass-through `sctrcontrol` fields other than S, U, and STE.  This would simplify behavior on traps and trap returns between V=0 and V=1, since those shared CTR configuration fields would not change.  But this would be undesirable for host + guest usages, since it would require the host to switch `sctrcontrol` on each host/guest transition._
====
