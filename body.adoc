...
The mctrcontrol register enables and configures the CTR capability while V=0.  Reset value is 0.
...
|WRAP   |TOS has wrapped.  Set when TOS has value depth-1 (where depth is based on ctrcontrol.DEPTH), and a CTR update causes TOS to be incremented.  Cleared when TOS has value zero and a CTR update causes TOS to be decremented (which only happens on returns when RASEMU=1), and on CSR writes that set ctrcontrol.CLR.
