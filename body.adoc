...
|Custom[3:0] | WARL bits designated for custom use.  The reset value must be 0, corresponding to standard behavior.  See <<Custom Extensions>>.
|===
...
Any custom CTR extension must be associated with a non-zero value within the designated custom bits in `__x__ctrctl`.  The reset value of these custom bits must be zero.
